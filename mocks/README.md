...
계약 원본은 Notion [openapi.yaml (v0.3)](https://app.notion.com/p/3b45f62e86848168936fe869152d748d) 2026-08-10 갱신본입니다
(사람용 상세는 [API 명세서](https://app.notion.com/p/f3f5f62e868482ee9faf816de775057c) 쪽).
...
| `kd7m2p` | ENDED | 공개 리포트. 리포트가 GENERATED·`isPublic=true`, 미분류 1건 포함            |
...
붙일 때만 동작하는 경로입니다. 그래서 에러 코드 표에는 `CSRF_TOKEN_INVALID`가 있지만
목은 이 코드를 내지 않습니다 — 실제 BE에 붙였을 때만 나옵니다.

## 알아둘 것

- **`toxic`은 욕설 사전 매칭입니다.** 인신공격·비하는 포함하지 않습니다(#160).
  브라우저 Web Worker가 `lib/tagger/tagger.js`의 사전으로 판정해서 요청 body에 담아 보냅니다.
  seed의 `toxic` 값도 이 정의를 따릅니다 — 욕설 어휘가 없으면 아무리 공격적인 문장이어도 `false`입니다.
  판단이 필요한 소감은 주최자가 모더레이션에서 수동으로 숨깁니다.

- **`toxic: true`면 제출 시점부터 `HIDDEN`으로 저장됩니다**(#150). 실시간 지표에 욕설이 뜨지 않게
  하는 것이 목적이고, 독성 건수 자체는 계속 셉니다.
