...
// worker.js 와 scripts/tagger-eval.mjs 가 이 파일을 공유한다.
// (스파이크에서는 sweep.mjs 가 이 자리다. 두 벌을 같이 맞출 것.)
...
// "-나요"는 의문 종결 어미와 동사 '나다'의 평서 종결이 겹친다.
// "공유되나요"는 질문이지만 "기억이 안 나요"는 평서문이다.
// 물음표가 있으면 아래에서 먼저 잡히므로 여기까지 오지 않는다.
const DECLARATIVE_NAYO_RE = /(안|못)\s*나요[\s.!~]*$/;

export function isQuestion(text) {
  const t = text.trim();
  if (!t) return false;
  if (t.includes('?')) return true;
  if (!DECLARATIVE_NAYO_RE.test(t) && QUESTION_ENDING_RE.test(t)) return true;
...
const PROFANITY_ALLOWLIST = [
  ...
  '시발자',
  '개새벽',
  // '개같'은 앞 단어의 끝 글자 '개'와 '같이'가 붙어 만들어진다. 어간만 넣으면
  // 번개같이·번개같은·번개같아를 모두 커버한다.
  '번개같',
  '무지개같',
  '안개같',
];
...
export function isToxic(text) {
  ...
  // 정상 단어를 걷어낸 뒤 검사한다. 정규화 뒤에 걷어내야 한다 — 정규화가 공백을
  // 지우므로, "번개 같이"는 정규화 전에는 allowlist에 걸리지 않고 공백이 지워진
  // 다음 "번개같이"가 되어 '개같'에 잡힌다.
  let norm = normalizeForProfanity(raw);
  for (const ok of PROFANITY_ALLOWLIST) norm = norm.split(normalizeForProfanity(ok)).join('');
