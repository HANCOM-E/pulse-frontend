...
`negative`·`warning`은 조건형, `info`는 상시형입니다.

조건형에 닫기 버튼을 달면 등록이 여전히 실패한 상태인데 배너만 사라져서, 사용자가 해결된 줄 알고 넘어갑니다. 입력 폼의 오류 메시지에 X 버튼을 안 다는 것과 같은 이유입니다. 상시형은 애초에 사라질 이유가 없습니다.
...
`type`에 기본값이 없습니다. 셋 중 "보통 이것"이라 할 게 없어서, 안 쓰면 타입 에러가 나도록 필수로 두었습니다.
...
테두리는 **점선**입니다. 기능적 이유는 없고 시안 그대로입니다. 다른 컴포넌트(Input·Chip·Badge)는 실선이라 Banner만 예외이니 통일하지 마세요.
...
| 애니메이션      | 등장 페이드 + 0.5rem 위로 · 퇴장 페이드, 각 0.2초 |

`prefers-reduced-motion`이면 **등장** 애니메이션이 생략되고 바로 나타납니다. 퇴장 페이드는 그대로 돕니다 — 불투명도만 바뀌고 이동이 없어서, 모션을 줄이는 설정이 막으려는 대상이 아닙니다.

덕분에 완전히 보이는 시간이 설정과 무관하게 정확히 4초입니다.
